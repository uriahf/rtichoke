...
prevalence_one_model <- rtichoke:::get_prevalence_from_performance_table(performance_table_one_model)
...
data.frame(population = rep(names(prevalence), each = 2), 
           ...
           col = rep(col_values, each = 2)) %>%
  arrange(population, x, y)

# gains multiple populations


data.frame(population = names(prevalence), x = prevalence, y = 1, col_values, row.names = NULL) %>%
  bind_rows(
    data.frame(population = rep(names(prevalence), each = 2),
               x = c(0, 1),
               y = c(0, 1),
               col_values = rep(col_values, each = 2)
               )
  ) %>%
  arrange(population, x, y)
...
prevalence
data.frame(population = names(prevalence), x = prevalence, y = 0, col_values, row.names = NULL) 
