---
title: "Performance Table Examples"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{performance_table_examples}
...
It is useful to work with performance tables for most functions in {rtichoke}.

There are several different cases for different performance tables:
...
one_pop_one_model_as_a_vector <- create_performance_table(
  probs = example_dat$estimated_probabilities,
  real = example_dat$outcome)

head(one_pop_one_model_as_a_vector)

one_pop_one_model_as_a_vector_enforced_percentiles_symmetry <- create_performance_table(
  probs = example_dat$estimated_probabilities,
  real = example_dat$outcome,
...
Or alternatively a list with a single vector. In that case a column named "model" will be added to the performance table, it is recommended to name the vectors:

```{r}

one_pop_one_model_as_a_list <- create_performance_table(
  probs = list("Logistic Regression" = example_dat$estimated_probabilities),
  real = example_dat$outcome)

head(one_pop_one_model_as_a_list)

one_pop_one_model_as_a_list_enforced_percentiles_symmetry <- create_performance_table(
  probs = list("Logistic Regression" = example_dat$estimated_probabilities),
  real = example_dat$outcome,
  enforce_percentiles_symmetry = T)
...
one_pop_three_models <- create_performance_table(
...
one_pop_three_models_enforced_percentiles_symmetry <- create_performance_table(
  probs = list("Good Model" = example_dat$estimated_probabilities,
               "Bad Model" = example_dat$bad_model,
               "Random Guess" = example_dat$random_guess),
  real = example_dat$outcome, 
...
One needs to use a list with a vector for each model as an input for the `probs` argument and for the `real` argument. A column named "population" will be added to the performance table, it is recommended to name the vectors:

```{r}

train_and_test_sets <- create_performance_table(
...
train_and_test_sets_enforced_percentiles_symmetry <- create_performance_table(
  probs = list("train" = example_dat %>%
                 dplyr::filter(type_of_set == "train") %>%
                 dplyr::pull(estimated_probabilities),
               "test" = example_dat %>% dplyr::filter(type_of_set == "test") %>%
