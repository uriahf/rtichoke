...
vignette: >
  %\VignetteIndexEntry{Getting Started with rtichoke}
...
For methodological intuition, mathematical derivations, and statistical theory, visit the [rtichoke blog](https://rtichoke-blog.netlify.app/).

---

## The `rtichoke` Input Structure

`rtichoke` is model-agnostic. It does not fit models or require specific model objects (e.g., `glm` or `randomForest`). Instead, it works directly with vectors of **predicted probabilities** (`probs`) and **observed binary outcomes** (`reals`).
...
When evaluating one model across distinct populations (such as training vs. validation sets or demographic subgroups), supply named lists for both `probs` and `reals`:
...
Functions prefixed with `create_` (e.g., `create_roc_curve()`, `create_decision_curve()`, `create_performance_table()`) accept raw `probs` and `reals` directly and compute performance metrics on the fly.
...
- [Discrimination Guide](discrimination.html) — ROC, PR, Gains, and Lift curves.
- [Calibration Guide](calibration.html) — Binned and smooth calibration analysis.
- [Clinical Utility Guide](clinical-utility.html) — Decision Curves and Interventions Avoided.
- [Performance Tables Guide](performance-tables.html) — Interactive tables and reports.
- [Recipes & Workflows](recipes-and-workflows.html) — Copy-paste cheatsheet for common workflows.
