...
Curves that are generated from performance metrics can be called by `plot_*_curve()` or `create_*_curve` from performance table.
...
Interactive performance table is just a `reactable` object that wraps a performance table and returns an HTML object..
