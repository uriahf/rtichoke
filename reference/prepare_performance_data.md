...
Sometimes, instead of evaluating performance at fixed probability
thresholds, performance is evaluated across rank-based operating points
in the population. In `rtichoke`, this is requested with
`stratified_by = "ppcr"` (Predicted Positives Condition Rate, PPCR) and
can be interpreted in terms of prediction percentiles. For example, to
evaluate model performance when targeting approximately the
highest-scoring 5% of a population, use the PPCR stratification. The
resulting performance data contain metrics evaluated across the
corresponding PPCR operating points.
